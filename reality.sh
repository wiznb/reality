...
status() {
    ...
    if [[ -n "$port" ]]; then
        res=$(ss -ntlp 2>/dev/null | grep "${port}" | grep xray)
        if [[ -z "$res" ]]; then
            echo 2
        else
            echo 3
...
preinstall() {
    $PMT clean all >/dev/null 2>&1
    [[ "$PMT" = "apt" ]] && $PMT update >/dev/null 2>&1
    echo ""
    echo "安装必要软件，请等待..."

    if [[ "$PMT" = "apt" ]]; then
        res=$(which ufw 2>/dev/null)
        [[ "$?" != "0" ]] && $CMD_INSTALL ufw >/dev/null 2>&1
    fi

    res=$(which curl 2>/dev/null)
    [[ "$?" != "0" ]] && $CMD_INSTALL curl >/dev/null 2>&1

    res=$(which openssl 2>/dev/null)
    [[ "$?" != "0" ]] && $CMD_INSTALL openssl >/dev/null 2>&1

    res=$(which qrencode 2>/dev/null)
    [[ "$?" != "0" ]] && $CMD_INSTALL qrencode >/dev/null 2>&1

    res=$(which jq 2>/dev/null)
    [[ "$?" != "0" ]] && $CMD_INSTALL jq >/dev/null 2>&1
...
getkey() {
    ...
    key_pair=$(parse_x25519_keys /usr/local/etc/xray/key)
    ...
    echo -n "$private_key" > /usr/local/etc/xray/privatekey
    echo -n "$public_key" > /usr/local/etc/xray/publickey
...
generate_link() {
    if [[ ! -f /usr/local/etc/xray/ip || ! -f /usr/local/etc/xray/port || ! -f /usr/local/etc/xray/uuid || ! -f /usr/local/etc/xray/servername || ! -f /usr/local/etc/xray/publickey || ! -f /usr/local/etc/xray/sid || ! -f /usr/local/etc/xray/name ]]; then
    ...
    colorEcho $BLUE "reality订阅链接：${LINK}"
    echo ""
    colorEcho $YELLOW "reality节点二维码（可直接扫码导入到 v2rayN、shadowrocket 等客户端）："
...
Modify_xrayconfig() {
    ...
    echo ""
    read -p "是否需要重新生成shortID（0：保持不变；1：重新生成）:" USER_SID_CHOICE
    if [[ $USER_SID_CHOICE == 1 ]]; then
        echo ""
        echo "正在重新生成shortID..."
        USER_SID=$(openssl rand -hex 8)
        echo "$USER_SID" > /usr/local/etc/xray/sid
...
start() {
    ...
    if [[ -z "$res" ]]; then
        colorEcho $RED "xray启动失败，请检查配置和端口是否被占用！"
...
Xray() {
    ...
    echo -e "  ${GREEN}6.${PLAIN}  修改reality配置"
    echo " -------------"
